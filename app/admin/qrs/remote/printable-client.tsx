...
export default function PrintableRemoteLandingQr({ config }: { config: Config }) {
  ...
  useEffect(() => {
    const origin =
      typeof window !== "undefined"
        ...
        : "";
    const url = `${origin}/donate/remote?qr=${encodeURIComponent(config.qrToken)}`;
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "remote-all-charities",
    pageStyle: `
      ...
      >
        <header className="flex items-start justify-between gap-3 print:hidden">
          <div>
            ...
            <div className="text-sm text-zinc-600">All charities</div>
          </div>
          <div className="flex gap-2">
            {qrDataUrl ? (
              <a
                href={qrDataUrl}
                download="remote-all-charities-qr.png"
        ...
        <div className="overflow-hidden rounded-2xl border border-black/10 shadow-sm print:border-none print:shadow-none break-inside-avoid">
          <div className="grid gap-0 md:grid-cols-2">
            <div className="flex flex-col gap-3 bg-white p-6 break-inside-avoid">
              <div className="text-xs uppercase tracking-wide text-zinc-600">Rack Up</div>
